...
function Task({ task, onChange, onDelete }) {
  ...
  if (isEditing) {
    taskContent = (
    <div className='json-ld-container editing-tab-wrapper'>

<div className='editing-content'>
  <label className="faqAppLabel" htmlFor={`faq-input-${task.id}`}>
    Question:
    <input
      id={`faq-input-${task.id}`}
      name={`faq-input-${task.id}`}
      value={task.question}
      onChange={e => {
        onChange({
          ...task,
  ...
  <label className="faqAppLabel" htmlFor={`faq-textarea-${task.id}`}>
    Answer:
    <textarea
      id={`faq-textarea-${task.id}`}
      name={`faq-textarea-${task.id}`}
      onChange={e => {
        onChange({
          ...task,
    ...
    ></textarea>
        ...
        <div className='button-wrapper user-actions'>
          <button style={{'display': 'none'}} onClick={() => setIsEditing(false)}>
          <span className='visually-hidden'>Accept</span>
          ...
          <span className='visually-hidden'>Move</span>
